...
public class BallMesh : MonoBehaviour
{
    ...
    void DrawSphere(int subdivisions = 0, float radius = 1)
    {
        if (subdivisions > 4)
        {
            subdivisions = 4;
        ...
        vertices = new Vector3[(resolution + 1) * (resolution + 1) * 4 - 3 * (resolution * 2 + 1)];
    ...
    private void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < vertices.Length; i++)
        {
            ...
            Gizmos.DrawCube(transform.position + vertices[i], new Vector3(.1f, .1f, .1f));
        }
    }
}